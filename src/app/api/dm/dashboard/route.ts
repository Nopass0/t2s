...
import { getDmDashboard } from "@/server/queries/dashboard";
...
export async function GET(req: Request) {
  ...
  const monthRef = monthStartParam
    ...
    : undefined;
  const data = await getDmDashboard(db, userId, monthRef);

  if (!data) {
    return NextResponse.json({ error: "ДМ не найден" }, { status: 404 });
  }
